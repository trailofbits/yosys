...
The {\tt \$memrd} cells have a clock input \B{CLK}, an enable input \B{EN},
an asynchronous reset input \B{ARST}, a synchronous reset input \B{SRST}, an
...
A bitmask of write ports that this port is transparent with -- if the bit index corresponding
to the other port's \B{PORTID} is set here, this port's reads will show the other port's written
value when both ports are active simultanously at the same address; otherwise, this port's reads
will be unaffected by the other port's writes in the same cycle.  Setting this to 1 is only
valid for ports that share clock and clock polarity.

\item \B{INIT\_VALUE} \\
The initial value of the data output, for synchronous ports.

\item \B{CE\_OVER\_SRST} \\
If synchronous reset is used, this determines the relative priority of \B{SRST} and \B{EN} inputs.
When this parameter is non-zero, \B{EN} has priority, otherwise \B{SRST} has priority.

\item \B{ARST\_VALUE} \\
The value that \B{ARST} will reset the data output to, if enabled.

\item \B{SRST\_VALUE} \\
The value that \B{SRST} will reset the data output to, if enabled.
\end{itemize}
...
\item \B{WIDTH} \\
The number of data bits (width of the \B{DATA} output port).  Like with {\tt \$memrd} cells,
...
\item \B{PORTID} \\
The 0-based numeric identifier of this write port, used to identify it in \B{PRIORITY\_MASK}.
\end{itemize}

\item \B{PRIORITY\_MASK} \\
A bitmask of write ports that this port wins over in a write conflict -- if the bit index corresponding
to the other port's \B{PORTID} is set here, this port's writes win over the other port's writes
when both happen simultanously.  The priority relationship should be a partial order.  If no port
has priority over the other, the behavior of conflict is undefined.  Setting this to 1 is only valid
for ports that share clock and clock polarity.
\end{itemize}
...
\B{TRANSPARENCY\_MASK} values of the original {\tt \$memrd} cells.
...
\B{PRIORITY\_MASK} values of the original {\tt \$memwr} cells.
\end{itemize}
